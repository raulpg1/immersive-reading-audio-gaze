...
from config import AUDIO_OUTPUT_PATH, AUDIO_LDM_MODEL, AUDIO_NUM_INFERENCE_STEPS, AUDIO_GUIDANCE_SCALE
...
def generate_audioldm(prompt_struct):
    ...
    pipe = AudioLDMPipeline.from_pretrained(AUDIO_LDM_MODEL, torch_dtype=torch.float32)
    pipe = pipe.to("cuda")
    ...
    for i in prompt_struct.keys():
        for j in range(len(prompt_struct[i])-1):
            ...
            print(f"[AUDIOLDM] Generando audio del parrafo {i}: linea: {linea_act+linea} Prompt: {prompt_struct[i][j]['prompt']}")

            audio = pipe(prompt = prompt_struct[i][j]["prompt"], 
                         num_inference_steps = AUDIO_NUM_INFERENCE_STEPS, 
                         audio_length_in_s = prompt_struct[i][j]["length"], 
                         guidance_scale = AUDIO_GUIDANCE_SCALE
                         ).audios[0]
            
            audio_output_file_path = os.path.join(AUDIO_OUTPUT_PATH,f"{linea_act+linea}.wav")

            scipy.io.wavfile.write(audio_output_file_path, 
                                   rate=16000, 
                                   data=audio)
